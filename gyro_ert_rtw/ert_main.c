...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) (gyro_M->Timing.clockTick0+
    gyro_M->Timing.clockTickH0* 4294967296.0);
  gyro_step();
...
int main(void)
{
  ...
  errorCode = extmodeInit(gyro_M->extModeInfo, &rtmGetTFinal(gyro_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(gyro_M, true);
    }
  }
