...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) (gyrotest_M->Timing.clockTick0+
    gyrotest_M->Timing.clockTickH0* 4294967296.0);
  gyrotest_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(gyrotest_M, 0);
  ...
  errorCode = extmodeInit(gyrotest_M->extModeInfo, &rtmGetTFinal(gyrotest_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(gyrotest_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(gyrotest_M);
